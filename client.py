...
def attempt_login():
    ...
    try:
        response = requests.post(
            f"{API_URL}/login", data={"username": username, "password": password}
        )
        ...
        if response.status_code == 200:
            ...
            messagebox.showerror(
                "Ошибка", f"Неизвестная ошибка: {response.status_code}"
            )
...
def open_main_window():
    ...
    def update_quantity():
        ...
        tk.Label(
            update_window, text=f"Товар: {item_name}", font=("Arial", 10, "bold")
        ).pack(pady=10)
        tk.Label(
            update_window, text="На сколько изменить? (например, 5 или -3): "
        ).pack()
        ...
        tk.Button(
            update_window, text="Сохранить", bg="lightgreen", command=save_quantity
        ).pack(pady=10)
    ...
    def add_new_item():
        ...
        tk.Label(add_window, text="Наименование: ", font=("Arial", 10)).pack(
            pady=(10, 0)
        )
        ...
        def save_new_item():
            ...
            if not name:
                messagebox.showwarning(
                    "Внимание", "Название товара не может быть пустым!"
                )
                return
            try:
                price = float(price_text)
                headers = {"Authorization": f"Bearer {current_token}"}
                new_item_data = {"name": name, "price": price}
                response = requests.post(
                    f"{API_URL}/items", json=new_item_data, headers=headers
                )
        ...
        tk.Button(
            add_window, text="Добавить", bg="lightblue", command=save_new_item
        ).pack(pady=15)
    ...
    def delete_item():
        ...
        confirm = messagebox.askyesno(
            "Подтверждение",
            f"Вы уверены," "что хотите безвозвратно удалить товар?" f"'{item_name}'?",
        )
    ...
    btn_update = tk.Button(
        top_frame, text="Изменить остаток", bg="lightgreen", command=update_quantity
    )
    btn_update.pack(side=tk.RIGHT, padx=5)

    btn_add = tk.Button(
        top_frame, text="Новый товар", bg="lightblue", command=add_new_item
    )
    btn_add.pack(side=tk.RIGHT)

    btn_sync = tk.Button(
        top_frame, text="Синхронизация", bg="lightyellow", command=sync_with_supplier
    )
    btn_sync.pack(side=tk.RIGHT, padx=5)

    btn_delete = tk.Button(
        top_frame, text="Удалить", width=10, bg="lightcoral", command=delete_item
    )
...
tk.Button(root, text="Войти", command=attempt_login, width=15, bg="lightblue").pack(
    pady=20
)
